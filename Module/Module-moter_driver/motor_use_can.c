 ...
 *******************************************************************************
  ...
  函数列表   : 1) HAL_CAN_RxFifo0MsgPendingCallback() 【HAL库函数，CAN回调函数】
							 //CAN发送
							 2) Gimbal_Motor6020()									【外部调用：使用处】  //一般在任务中调用
...
{
	CAN_RxHeaderTypeDef  CAN1_Rx_Header; //CW:接收回调函数只有一个，辨识不同电机时通过电机的ID号
	...
	if(hcan == &hcan1)
	{
		...
		switch(CAN1_Rx_Header.StdId)
		{
			...
			{		
				...
				if(motor_get[CHASSIS_3508MOTOR].msg_cnt++ <= 50)	//CW:CAN接收数据的次数小于50
				{
					Get_Moto_Offset(&motor_get[CHASSIS_3508MOTOR],CAN1_RX_date); //	获取电机返回值的偏差值(获取电机上电时的角度值)
				}
				else
				{	
...
/**
  ...
  * @retval				
*/
HAL_StatusTypeDef Gimbal_Motor6623_Calibration(CAN_HandleTypeDef * hcan) //CW:不明白该函数的作用
{
	static CAN_TxHeaderTypeDef  Cloud_Platform_Data;
	uint8_t CAN_TX_DATA[8];
...
void Get_Moto_Measure_3508(motor_measure_t *ptr,uint8_t CAN_RX_date[])
{
	...
	if(ptr->angle - ptr->last_angle > 4096)
		...
		ptr->round_cnt ++;
	ptr->total_angle = ptr->round_cnt * 8192 + ptr->angle - ptr->offset_angle;  //减去偏差值
}
